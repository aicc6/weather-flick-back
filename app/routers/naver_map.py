...
async def search_places(
    ...
    location: Optional[str] = Query(None, description="위치 (위도,경도)"),
...
async def get_nearby_places(
    ...
    """주변 장소 검색"""
    places = await naver_map_service.get_nearby_places(latitude, longitude, radius, category)
    return {
        "places": places,
        "total": len(places),
...
async def get_nearby_restaurants(
    ...
    """주변 맛집 검색"""
    restaurants = await naver_map_service.search_restaurants_nearby(latitude, longitude, radius)
    return {
...
async def get_nearby_transportation(
    ...
    """주변 교통 정보 검색"""
    transportation = await naver_map_service.search_transportation_nearby(latitude, longitude, radius)
    return {
...
async def get_map_embed_url(
    ...
    """네이버 지도 임베드 URL 생성"""
    embed_url = await naver_map_service.get_map_embed_url(latitude, longitude, zoom, width, height)
    return {
        "embed_url": embed_url,
...
async def get_static_map_url(
    ...
    """정적 지도 이미지 URL 생성"""
    static_url = await naver_map_service.get_static_map_url(latitude, longitude, zoom, width, height)
    return {
...
async def get_map_widget_html(
    ...
    """네이버 지도 위젯 HTML 생성"""
    widget_html = await naver_map_service.get_map_widget_html(latitude, longitude, zoom, width, height)
    return {
        "html": widget_html,
        "coordinates": {"latitude": latitude, "longitude": longitude},
        "zoom": zoom,
...
async def search_with_coordinates(
    ...
    """좌표 기반 장소 검색"""
    places = await naver_map_service.search_with_coordinates(latitude, longitude, query, limit)
    return {
        "places": places,
        "total": len(places),
        "query": query,
...
async def get_search_categories():
    """검색 카테고리 목록"""
    categories = [
        "맛집", "카페", "호텔", "펜션", "게스트하우스", "모텔", "리조트",
        "지하철역", "버스정류장", "공항", "기차역", "관광지", "쇼핑몰",
        "병원", "약국", "은행", "편의점", "주유소", "주차장"
    ]
