...
from app.models import User, WeatherRequest, WeatherResponse, ForecastResponse
...
async def get_current_weather_by_city(
    city: str,
    country: Optional[str] = Query(None, description="국가 코드 (예: KR, US)")
):
    """도시명으로 현재 날씨 조회"""
    try:
        weather_data = await weather_service.get_current_weather(
...
async def get_favorites_weather(current_user: User = Depends(get_current_active_user)):
    ...
    return {
        "user_id": current_user.id,
        "favorites_weather": weather_data
    }
