...
async def get_short_forecast_kma(
    ...
    coords = get_city_coordinates(city)
    forecast_data = await kma_weather_service.get_short_forecast(coords["nx"], coords["ny"])
...
async def get_mid_forecast_kma(
    ...
    if not reg_id:
        raise HTTPException(status_code=400, detail=f"중기예보를 지원하지 않는 도시입니다: {city}")
...
async def compare_weather_sources(
    ...
    # 기상청 데이터 조회
    coords = get_city_coordinates(city)
    kma_weather = await kma_weather_service.get_current_weather(coords["nx"], coords["ny"])
