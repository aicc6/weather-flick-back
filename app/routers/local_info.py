...
from app.models import (
    User, RestaurantResponse, TransportationResponse, AccommodationResponse,
    CityInfoResponse, SearchRequest, SearchResult, FavoritePlaceResponse,
...
async def search_restaurants(
    ...
    category: Optional[str] = Query(None, description="음식 카테고리 (한식, 중식, 일식, 양식, 카페, 기타)"),
...
async def search_transportation(
    city: str = Query(..., description="도시명"),
    region: Optional[str] = Query(None, description="지역명"),
    transport_type: Optional[str] = Query(None, description="교통 수단 타입 (지하철, 버스, 택시, 기차, 공항)"),
    limit: int = Query(20, description="결과 개수", ge=1, le=100)
):
    """교통 정보 검색"""
...
async def search_accommodations(
    city: str = Query(..., description="도시명"),
    region: Optional[str] = Query(None, description="지역명"),
    accommodation_type: Optional[str] = Query(None, description="숙소 타입 (호텔, 펜션, 게스트하우스, 모텔, 리조트)"),
    limit: int = Query(20, description="결과 개수", ge=1, le=100)
):
    """숙소 정보 검색"""
...
async def get_place_reviews(
    ...
    reviews = db.query(Review, User.username).join(
        User, Review.user_id == User.id
    ).filter(
        Review.place_id == place_id,
        Review.place_type == place_type
