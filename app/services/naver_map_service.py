...
class NaverMapService:
    ...
    async def search_places(self, query: str, location: str = None, category: str = None,
    ...
    async def get_route_guidance(self, start: str, goal: str,
    ...
    async def get_nearby_places(self, latitude: float, longitude: float,
                               radius: float = 1000, category: str = None) -> List[Dict]:
        """주변 장소 검색"""
        if not self.client_id or not self.client_secret:
            return []
    ...
    async def get_map_embed_url(self, latitude: float, longitude: float,
                               zoom: int = 15, width: int = 600, height: int = 400) -> str:
        """네이버 지도 임베드 URL 생성"""
        return f"https://map.naver.com/v5/embed/place/{latitude},{longitude}?zoom={zoom}&width={width}&height={height}"

    async def get_static_map_url(self, latitude: float, longitude: float,
                                zoom: int = 15, width: int = 600, height: int = 400) -> str:
        """정적 지도 이미지 URL 생성"""
    ...
    async def search_restaurants_nearby(self, latitude: float, longitude: float,
                                      radius: float = 1000) -> List[Dict]:
        """주변 맛집 검색"""
        return await self.get_nearby_places(latitude, longitude, radius, "맛집")

    async def search_hotels_nearby(self, latitude: float, longitude: float,
                                 radius: float = 1000) -> List[Dict]:
        """주변 숙소 검색"""
        return await self.get_nearby_places(latitude, longitude, radius, "호텔")

    async def search_transportation_nearby(self, latitude: float, longitude: float,
                                         radius: float = 1000) -> List[Dict]:
        """주변 교통 정보 검색"""
    ...
    async def get_map_widget_html(self, latitude: float, longitude: float,
                                 zoom: int = 15, width: int = 600, height: int = 400) -> str:
        """네이버 지도 위젯 HTML 생성"""
        return f"""
    ...
    async def search_with_coordinates(self, latitude: float, longitude: float,
